...
analytics remain future schema work. Fuel/DEF purchase tracking, end odometer,
...
behavior. Current code now exposes those calculator planning fields in the form,
stores them in `input_snapshot`, displays route duration in human-readable form,
and surfaces planning-hour/benchmark context in results. First-class database
columns for schedule/window analytics remain future schema work only if
approved.
...
| Karpilo FSC Intelligence | Calculator FSC treatment, paid loaded miles, deadhead miles, existing route variance, MPG, and EIA/manual diesel price | Authenticated user for revenue inputs; public EIA data for diesel baseline | Calculator engine, Karpilo Atlas Intelligence Readout, saved result snapshots | `input_snapshot`, `result_snapshot`; no first-class schema in this branch | Existing EIA fuel baseline, calculator mileage inputs, Route Intelligence variance when already available | CONNECTED through snapshots |
...
| Calculator timing/window assumptions | Calculator schedule/time planning fields | Authenticated user | Calculator engine, Route Intelligence context, saved-load snapshots, detail/report context | Current `input_snapshot`; future first-class columns only if approved | Route duration minutes, 50 mph benchmark, 10-hour planning day, user overrides | CONNECTED through snapshots |
| Calculator load weight | Visible calculator cargo weight input | Authenticated user | Calculator engine, Atlas route context, saved-load detail/report, future truck-specific routing context | `saved_loads.estimated_load_weight_lbs`, `input_snapshot`, optional `equipment_context_snapshot` | Estimated-only route/legal boundary applies | CONNECTED |
...
| Route estimated loaded/deadhead/total miles | Google route estimate action | `input_snapshot.routeEstimate`, `routeLoadedMiles`, `routeDeadheadMiles` | User/app | Calculator route estimate action | Results panel mileage intelligence, saved-load detail via snapshots, route model | Estimated miles stay separate from paid loaded miles; copy to paid loaded miles remains explicit; display-only route miles belong to results/readout context | CONNECTED |
| Route estimated time | Google route estimate action | `input_snapshot.routeEstimate.*DurationMinutes`, `googleRouteDurationHuman`, `googleRouteDurationQuarterHours` | App provider estimate | Calculator route estimate action and planning suggestion action | Results panel mileage intelligence and planning suggestions | UI displays human-readable duration and 0.25-hour planning context; calculator keeps suggestion application while duration display belongs to results/readout context | CONNECTED |
...
| Stop dates, windows, and dwell | Calculator stop editor | `input_snapshot.routeStops[]`; first-class columns only if approved later | User | Calculator timing UI | Calculator planning days, saved-load detail/report context, future AI/Atlas schedule analysis | Default dwell is 2.00 hours; dates without exact time/window are treated as broad planning windows | CONNECTED through snapshots |
| Appointment window start/end context | Calculator pickup, delivery, and stop window editors | `input_snapshot` endpoint fields and `input_snapshot.routeStops[]`; first-class columns only if approved later | User | Calculator timing UI | Calculator planning context, saved-load detail/report context, future schedule analysis | Start/end dates persist separately from optional 24-hour start/end times; open-ended means flexible time inside dated context, not missing date context | CONNECTED through snapshots |
...
| Planning hours/days and user overrides | Calculator schedule/time planning fields and generated suggestions | `input_snapshot`, `result_snapshot` | User | Calculator timing model and results mileage intelligence | Profit/day, hourly profitability, revenue leakage analysis | Generated suggestions use Google minutes when available, otherwise 50 mph and 10-hour planning days; edited user values remain authoritative; benchmark displays belong to results/readout context | CONNECTED |
...
| Revenue mode/gross/RPM/FSC | Calculator form | `saved_loads`, `input_snapshot`, `result_snapshot` | User | Calculator | Engine, results, history/detail/report | Gross, linehaul, revenue per mile, net; Karpilo FSC Intelligence separates actual, built-in, separate-missing, or unknown FSC treatment | CONNECTED |
...
| Accessorial/toll/lumper actuals | Load detail post-trip actuals form | `actuals_snapshot`, `post_trip_actuals` | User | Saved-load detail | Detail/report actuals | Actual gross revenue and post-trip expense rows capture settlement and trip-cost actuals; calculator submits these as zero/empty planning values | CONNECTED through snapshots |
...
3. `fix/loadiq-calculator-time-window-schema`
   - Optional future branch only if first-class schedule/window analytics,
     filtering, or report queries require database columns beyond snapshots.
   - Do not add schema until the snapshot-backed V1 behavior proves the query
     need.
