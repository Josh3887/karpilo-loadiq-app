...
export async function duplicateSavedLoad(loadId: string) {
  ...
  const duplicatePayload = {
    ...
    load_status_reason: savedLoad.load_status_reason ?? null,
    ...
    fuel_gauge_snapshot: savedLoad.fuel_gauge_snapshot ?? {},
    equipment_context_snapshot: savedLoad.equipment_context_snapshot ?? {},
  ...
  if (
    duplicateInsert.error &&
    isMissingSavedLoadSnapshotColumnError(duplicateInsert.error)
  ) {
    duplicateInsert = await supabase
      .from("saved_loads")
      .insert(omitSavedLoadSnapshotColumns(duplicatePayload))
      .select("id")
      .single();
  }

  if (duplicateInsert.error || !duplicateInsert.data) {
    throw new Error(formatSupabaseError(duplicateInsert.error));
  }

  const duplicate = duplicateInsert.data;
