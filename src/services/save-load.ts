...
import {
  isMissingSavedLoadSnapshotColumnError,
  omitSavedLoadSnapshotColumns,
  SAVED_LOAD_SNAPSHOT_SCHEMA_WARNING,
} from "@/services/saved-load-schema-compatibility";
...
export async function saveLoad({
  ...
  const savedLoadPayload = {
    ...
    reserve_allocation_percent:
      reserveAllocationMode === "percent" ? result.reserveAllocationValue : null,
  ...
  let savedLoadWarning: string | undefined;
  let savedLoadInsert = await supabase
    .from("saved_loads")
    .insert(savedLoadPayload)
    .select("id")
    .single();

  if (
    savedLoadInsert.error &&
    isMissingSavedLoadSnapshotColumnError(savedLoadInsert.error)
  ) {
    savedLoadInsert = await supabase
      .from("saved_loads")
      .insert(omitSavedLoadSnapshotColumns(savedLoadPayload))
      .select("id")
      .single();
    savedLoadWarning = SAVED_LOAD_SNAPSHOT_SCHEMA_WARNING;
  }

  if (savedLoadInsert.error || !savedLoadInsert.data) {
    throw new Error(formatSupabaseError(savedLoadInsert.error));
  }

  let routeStopWarning: string | undefined;
  const savedLoad = savedLoadInsert.data;
  ...
  const warnings = [savedLoadWarning, routeStopWarning].filter(Boolean);

  return {
    id: savedLoad.id as string,
